...
object helpers {
  ...
  implicit final class CursorExtended[F[_] : Async, T](private val c: Cursor[T] with WithOps[T]) {
    def headOptionF(implicit ec: ExecutionContext): F[Option[T]] = Async[F].fromFutureDelay(c.headOption)


    def toStream(implicit ec: ExecutionContext): F[Stream[F, T]] = {
      val stream = Stream.unfoldEval(c)(e => OptionT(e.headOptionF).map((_, e)).value)
      Sync[F].delay(stream)
    }


    def toStream2(implicit ec: ExecutionContext): F[Stream[F, T]] = {
      Async[F] fromFutureDelay c.foldWhile(Stream.empty: Stream[F, T])(
            ...
            c.foldBulksM(())(
              { (x, xs) =>
                ...
                enqueue(chunk.some).flatMap {
                  case Left(_) => enqueue(None).map(Cursor.Done(_))
