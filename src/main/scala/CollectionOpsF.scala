...
object CollectionOpsF {
  implicit final class GenColExt[P <: SerializationPack](val collection: GenericCollection[P]) extends AnyVal {
    ...
    def findAndModifyF[F[_]: Async, S](
      ...
      Async[F].fromFutureDelay(collection.findAndModify(
        selector, modifier, sort, fields, bypassDocumentValidation,
        writeConcern, maxTime, collation, arrayFilters
      ))
    ...
    def findAndRemove[F[_]: Async, S](
      ...
      Async[F].fromFutureDelay(collection.findAndRemove(
        selector, sort, fields, writeConcern,
        maxTime, collation, arrayFilters
      ))
    ...
    def findAndUpdate[F[_]: Async, S, T](
      ...
      Async[F].fromFutureDelay(collection.findAndUpdate(
        selector, update, fetchNewObject, upsert, sort,
        fields, bypassDocumentValidation, writeConcern, maxTime, collation, arrayFilters
      ))
