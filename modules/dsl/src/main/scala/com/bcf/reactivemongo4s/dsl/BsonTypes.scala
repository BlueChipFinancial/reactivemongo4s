...
object BsonTypes {
  ...
  def numberOf[T <: BSONValue: TypeTag]: Int =
    typeOf[T] match {
...
//    case t if t =:= typeOf[BSONDBPointer] => 12 deprecated
      ...
      case t if t =:= typeOf[BSONDecimal]      => 19
      case t if t =:= typeOf[BSONMinKey.type]  => -1
