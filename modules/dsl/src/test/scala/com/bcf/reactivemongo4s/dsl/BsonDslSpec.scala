...
class BsonDslSpec extends AnyFlatSpec with Matchers {
  ...
  it should "create $elemMatch" in {
    ...
    val expected = BSONDocument("array" -> BSONDocument("$elemMatch" -> BSONDocument("value1" -> 1, "value2" -> BSONDocument("$gt" -> 1))))
    dsl shouldBe expected
  }
  ...
  it should "create $setOnInsert" in {
    ...
    val expected = BSONDocument("$setOnInsert" -> BSONDocument("defaultQty" -> 500, "inStock" -> true), "$set" -> BSONDocument("item" -> "apple"))
